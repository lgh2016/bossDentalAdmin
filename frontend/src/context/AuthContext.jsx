...
// Demo paciente — sigue funcionando contra mocks mientras los demás roles ya usan la API real.
const DEMO_PATIENT_EMAILS = new Set(
  DEMO_USERS.filter((u) => u.role === ROLES.PACIENTE).map((u) => u.email.toLowerCase()),
);
...
export function AuthProvider({ children }) {
  ...
  const login = useCallback(async (email, password) => {
    ...
    try {
      // ---- BYPASS DEMO PATIENT ----
      // El rol PATIENT sigue usando mocks (no autenticación real todavía).
      if (DEMO_PATIENT_EMAILS.has((email || "").toLowerCase())) {
      ...
      try {
        ...
        // Cualquier otro error técnico del backend (500, 502, etc.) → mensaje amigable.
        const friendly = new Error("No fue posible iniciar sesión. Verifica tus credenciales.");
        friendly.code = "LOGIN_FAILED";
        throw friendly;
      }
    } finally {
      setLoading(false);
